...
local GarageTaxi = {
    ...
    Events = {
        onSelected = function(self, _, btn, PMenu, menuData, result, slide)
            if btn.name == "Peugeot 406" then
                spawnCar('taxi')
            elseif btn.name == "Limousine" then
                spawnCar("tofaslimo")
            end
        end
    },
    ...
    Menu = {
        ["Les Véhicules : "] = {
            b = {
                ...
                {name = "Peugeot 406", askX = true},
                {name = "Limousine", askX = true},
            }
        }
    }
}
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if dist <= 5.0 and ESX.PlayerData.job.name == 'taxi' then
